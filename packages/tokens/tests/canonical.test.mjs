...
test("generated canonical JSON contains core token groups", () => {
  ...
  assert.ok(canonical.tokens.component);
...
test("generated token quality report explains contract health", () => {
  ...
  assert.equal(tokenQuality.modes.componentColors.missing.length, 0);
...
test("generated token docs are consumable without scraping CSS", () => {
  ...
  const componentButtonGroup = tokenDocs.groups.find((group) => group.name === "component.button");
  assert.ok(componentButtonGroup);
  assert.ok(
    componentButtonGroup.tokens.some(
      (token) =>
        token.name === "component.button.primary.high.background" &&
        token.type === "component" &&
        token.valueType === "color" &&
        token.cssVariable === "--ds-component-button-primary-high-background" &&
        token.value.light === "#155E75" &&
        token.value.dark === "#0E7490"
    )
  );
});
